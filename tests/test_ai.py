...
@patch("ai._chat_completion")
def test_ask_ai(mock_chat):
    mock_chat.return_value = "A database is an organized collection of data."
...
def test_ask_ai_with_context(mock_chat):
    mock_chat.return_value = "ACID properties ensure reliable transactions."
...
def test_generate_quiz(mock_chat):
    mock_chat.return_value = '{"questions": [{"question": "What is SQL?", "options": ["A", "B", "C", "D"], "answer": 0, "explanation": "SQL is structured query language"}]}'
...
def test_explain_mistake(mock_chat):
    mock_chat.return_value = "You selected option B, but option A is correct because..."
...
def test_ask_ai_chat(mock_chat):
    mock_chat.return_value = "Normalization helps reduce data redundancy."
    messages = [
...
def test_generate_quiz_from_material(mock_chat):
    mock_chat.return_value = '{"questions": [{"question": "What is deadlock?", "options": ["A", "B", "C", "D"], "answer": 1, "explanation": "Deadlock is circular wait."}]}'
...
def test_generate_flashcards(mock_chat):
    mock_chat.return_value = '{"flashcards": [{"front": "What is Mutex?", "back": "Mutual exclusion lock", "tag": "OS"}]}'
